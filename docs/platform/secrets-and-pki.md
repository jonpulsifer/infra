...
description: SOPS-encrypted cluster and host secrets, the OpenBao instance on folly, and the Terraform-issued FML certificate chain that signs ServiceAccount tokens.
---
...
`nix/secrets/bosun.sops.yaml` breaks the one-file-per-host pattern above on purpose: it holds bosun's own key on the shared kthx+bosun GitHub App (see [Bosun](../apps/bosun.md)), encrypted to every bosun host's recipient — riptide and oldschool — rather than a copy per host. A new bosun host needs its recipient added to this file through the same two-stage flow before its `bosun` unit can start, and what gets pasted in is this App key, never a personal access token.

The **two-stage recipient setup** (operator age key only on first commit, host's own ssh-to-age recipient added after first boot) and the operator-key location (`~/.config/age/keys.txt`, 1Password "sops homelab age key" — NOT the sops binary's default path) are spelled out in [Manage SOPS secrets](../runbooks/manage-sops-secrets.md).
...
Bootstrapping (init, unseal verification, first policies) is a runbook, not architecture: [Initialize OpenBao](../runbooks/initialize-openbao.md).
...
`terraform/argo/` is the root module for the `argocd` Terraform provider (`use_local_config = true`: it authenticates through a local ArgoCD session rather than an explicit token in the module). Atlantis applies it like any other root. The token/session wiring is scoped and rotated, not a shared static credential — see [How changes ship](how-changes-ship.md) for the apply flow and the runbook it points to for rotation.
