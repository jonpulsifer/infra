---
title: radiopi0
description: "A Raspberry Pi Zero W radio box with a pHAT BEAT DAC. It runs Raspbian; its NixOS config is unapplied."
specs:
  vendor: Raspberry Pi
...
In the flake as `nixosConfigurations.radiopi0` (`nix/hosts/radiopi0.nix` + `nix/hardware/pi0.nix`, shared with [blinkypi0](blinkypi0.md)). armv6l has no upstream binary cache and no board-support module, so [forge](forge.md) supplies its aarch64 cross-build platform — see [NixOS](../platform/nixos.md). Config is minimal: tailscale + ssh + wiringpi only.
