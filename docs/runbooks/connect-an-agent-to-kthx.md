...
description: "Mint an agent token and point an MCP client at kthx's /mcp endpoint, which serves every command as a tool."
---

kthx serves its command registry over the Model Context Protocol at `https://spindrift-control.lolwtf.dev/mcp`, so an agent can drive the platform — list apps, dispatch a build, deploy, roll back — through the same commands the UI dispatches. Unlike [Connect an agent to the wiki](connect-an-agent-to-the-wiki.md) this surface is **authenticated and it writes**. Every tool is an act.
