---
title: Apply a Terraform change
description: Edit an OpenTofu root, check it locally, and let Atlantis plan, apply and merge it from the PR.
---

Use this when changing Terraform under `terraform/` or cluster bootstrap Terraform under `clusters/<site>/bootstrap/`. Background lives in [OpenTofu and Atlantis](../platform/opentofu.md); the apply model (Atlantis autoplan → review → `atlantis apply` → automerge) is on [How changes ship](../platform/how-changes-ship.md).
...
Network facts come from the cluster topology single source of truth (`clusters/<site>/config/cluster-topology.json`) where a root already has a `topology.tf`; see [OpenTofu and Atlantis](../platform/opentofu.md).
