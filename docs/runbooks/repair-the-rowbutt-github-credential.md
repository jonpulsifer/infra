...
description: "Diagnose and fix the GitHub App credential mate mints sandbox tokens from: alerts, resync, key rotation and App reinstall."
---

Use this when a sandbox cannot push, when `MateGitHubCredentialBroken` or `MateGitHubTokenMintFailing` fires, or when the `clanky-bot[bot]` App's private key is rotated or its installation changed. mate holds the App's private key and mints installation access tokens from it; a sandbox holds a token and never the key. The design is on [Rowbutt](../apps/mate.md), the workload is `clusters/offsite/apps/mate/`, the minting is `apps/mate/src/github-app.ts`, and the alerts are `clusters/offsite/monitoring/mate-rules.yaml`.
...
Nothing here edits the live Secret. Flux owns the ExternalSecret and External Secrets owns the Secret; a hand-patched value is replaced without warning — see [Apply a Kubernetes change](apply-a-kubernetes-change.md).
...
The key never goes into git, into `docs/`, or into a terminal that logs. 1Password is the only copy; [Manage SOPS secrets](manage-sops-secrets.md) covers the other secret path in this repo and is not this one.

## Reinstalling the App

Install from the App's own page: **Install App → jonpulsifer → Only select repositories → infra**. Repository permissions are **Contents: Read and write**, **Pull requests: Read and write** and **Actions: Read-only**, and nothing else. The first two are the loop a turn ends with; Actions is what lets the agent read why its own pull request's checks failed rather than asking for the log. Every mint asks for exactly these three, so widening the installation later does not widen what a sandbox is handed.
