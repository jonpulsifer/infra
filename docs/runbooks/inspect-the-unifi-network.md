...
description: Run the read-only unifi.sh wrapper against the folly UDM Pro to compare live network state with terraform/network/unifi/.
---

Use this for read-only inspection of the live UniFi network before changing Terraform desired state. Desired state still lives under `terraform/network/unifi/`; changes apply through Atlantis. See [Apply a Terraform change](apply-a-terraform-change.md).
...
Author the desired change in Terraform and use [Apply a Terraform change](apply-a-terraform-change.md).
