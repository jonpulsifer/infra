...
description: "Install and debug windows_exporter, OhmGraphite and Vector on the Windows desktops that folly's Prometheus and VictoriaLogs collect from."
---
...
Both halves are in git. On the box, `dotfiles/windows/Install-Monitoring.ps1` installs the three agents and `dotfiles/windows/monitoring/` holds their configuration — the same pinned, hash-verified, idempotent shape the rest of the Windows desk uses, see [Install a Windows desktop](install-a-windows-desktop.md). In the cluster, `clusters/folly/monitoring/windows-exporters.yaml` is the scrape and the alerts, `clusters/folly/monitoring/victoria-logs-route.yaml` is the log endpoint, and `terraform/network/unifi/folly/windows-hosts.tf` holds the reservations.

Change those files, never the live box — see [How changes ship](../platform/how-changes-ship.md). Windows has no operator watching git, so "apply" here means re-running the installer; everything it does is idempotent.
