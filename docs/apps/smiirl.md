...
The counter's own web server answers on its iot address (find it by MAC on the controller, see [Inspect the UniFi network](../runbooks/inspect-the-unifi-network.md)):
...
Only the counter opens connections. Nothing needs to reach it from the cluster. No firewall rule gates the path either: the Gateway address is a Cilium LB VIP, which is in no UniFi zone, so iot traffic to it takes the gateway's `→ WAN` fall-through rather than an iot→Lab policy — see [Network](../platform/network.md).
...
DNS still points at the cloud: the quick check above must return the Gateway address on both capsule and spore. They pick up `main` on their nightly upgrade; to apply sooner, run the host's own upgrade unit (`sudo systemctl start nixos-upgrade.service`), one host at a time, as in [Deploy a NixOS host](../runbooks/deploy-a-nixos-host.md).
...
The Gateway is not programmed, or the pod is not running: `kubectl -n smiirl get gateway,pods`. Route and pod checks are on [Apply a Kubernetes change](../runbooks/apply-a-kubernetes-change.md).
