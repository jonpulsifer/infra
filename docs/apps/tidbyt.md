---
title: Tidbyt apps
description: Three Pixlet apps, rackstat, tempest and wishin, that the Tronbyt server on folly renders for the lab's Tidbyt pixel displays.
status: live
---

The Tidbyt apps are three Pixlet apps, rackstat, tempest and wishin, that draw screens on the lab's Tidbyt displays for the owner. A Tidbyt is an LED display of 64 by 32 pixels, and Pixlet renders its Starlark apps. Tronbyt, a self-hosted Tidbyt server on the folly [Kubernetes](../platform/kubernetes.md) cluster, renders the installed apps and serves the images to each display.
...
- Source: `apps/rackstat/`, `apps/tempest/` and `apps/wishin/`
