...
storage:: 256 GB Patriot P300 NVMe (installed and unused; root is a 4 GB tmpfs)
os:: NixOS 26.05 (Yarara), RAM image

- The Pi EEPROM HTTP-boots the signed `rackpi5` RAM image through [[Fleet/spore]] and runs entirely from RAM. The NVMe is detected but has no filesystem and is not part of the boot or runtime storage path.
