...
- # Terraform bootstrap
	...
	- ## The vessel project — `terraform/gcp/projects/bluenose/`
		- The home vessel. Its platform Terraform — the `terraform/modules/spindrift-vessel` and `terraform/modules/vessel-network` calls (enabled APIs, the `spindrift-runtime` service account the controller acts as, the controller's project roles, the Binary Authorization admission policy, the vessel VPC and private service connection), the `spindrift-controller` service account with its federation bindings, and the source bundle bucket — is in git history; reverting the vessel-retirement PR restores all of it at once. What the root carries today is each cluster's external-secrets read path into this project's Secret Manager (`iam.tf`), which stands on its own because the clusters deliver every ExternalSecret through it.
	...
	- ## Supply chain — `terraform/gcp/projects/trusted-builds/`
		- The KMS signing key (`kms.tf`), the `provenance` Binary Authorization attestor, and the `i` Artifact Registry repository. The installation manifest's `supplyChain` block names all three, which is how the running installation registers them.
