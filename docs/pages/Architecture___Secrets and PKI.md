...
- ## sops-nix (bare NixOS host secrets)
	...
	- `nix/secrets/bosun.sops.yaml` breaks the one-file-per-host pattern above on purpose: it holds bosun's own key on the shared Spindrift+bosun GitHub App (see [[Architecture/Bosun]]), encrypted to every bosun host's recipient — riptide, oldschool, and, once it has booted, `tender` — rather than a copy per host. A resurrected `tender` needs its recipient added to this file through the same two-stage flow before its `bosun` unit can start, and what gets pasted in is this App key, never a personal access token.
