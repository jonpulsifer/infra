...
- First install is a live install over the running RAM image: build the forge `sdImage` on spore (`nix build .#packages.x86_64-linux.forge`), `dd` the result to `/dev/nvme0n1` on the box, set EEPROM `BOOT_ORDER=0xf1276` by hand, reboot. Boot-order digits are tried right-to-left, so that reads NVMe, HTTP, network, SD, then restart the sequence — it prepends NVMe to the stock `0xf127` instead of replacing it, so a failed NVMe install falls back to the spore-published signed image rather than bricking.
