...
- # Decision
	...
	- Native targets are catalog entries derived from the same UniFi MAC inventory. The Nix image embeds its squashfs digest; a root-only publisher signs `boot.img` and atomically activates an immutable, content-addressed artifact set before Spore and nginx start.
- # Consequences
