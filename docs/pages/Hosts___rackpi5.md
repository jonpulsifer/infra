...
- The only EEPROM boot mode is HTTP (`BOOT_ORDER=0xf7`, `HTTP_HOST=10.2.0.11`, `HTTP_PATH=spore/api/native-boot/rackpi5`). Spore serves `boot.sig`, `boot.img`, and `nix-store.squashfs`; stage 1 verifies the squashfs digest anchored in the signed command line.
