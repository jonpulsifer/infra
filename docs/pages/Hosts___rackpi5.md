...
- The only EEPROM boot mode is HTTP (`BOOT_ORDER=0xf7`, `HTTP_HOST=10.2.0.11`, `HTTP_PATH=rackpi5-ram`). Spore serves `boot.sig`, `boot.img`, and `nix-store.squashfs` as plain static files under `/rackpi5-ram/`; stage 1 verifies the squashfs digest anchored in the signed command line.
