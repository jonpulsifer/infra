...
- ## Cloudflare Tunnel
	...
	- Generated App names under `lolwtf.dev` are published by external-dns from each App's own HTTPRoute: an unproxied A record at the shared Gateway's load-balancer address for a private reach, a proxied CNAME at the Spindrift tunnel for a public one. `cluster-gateway` carries a wildcard `*.lolwtf.dev` TLS listener admitting routes from `spindrift-apps`, which is why generated names are flat — a wildcard certificate binds one label. [[Architecture/Spindrift]] covers the reach and auth semantics.
	- A hand-managed wildcard CNAME still answers for every name in `lolwtf.dev`, and a Cloudflare Access application still covers it. Neither is Terraform-managed: the zone is adopted, and its records predate Terraform knowing about it. They are retired once Apps are observed publishing their own records.
