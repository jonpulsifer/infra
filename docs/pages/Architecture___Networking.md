...
- ## external-dns
	- `clusters/base/networking/external-dns/` runs external-dns against provider `cloudflare`, sourcing records from `crd`, `ingress`, and `gateway-httproute`, in `sync` policy with `txtOwnerId: ${CLUSTER_NAME}` (so folly and offsite don't fight over the same zone's TXT ownership records) and `domainFilters` scoped to the cluster's secret domain, `${GATEWAY_ZONE}`, and `${SPINDRIFT_DOMAIN}` — the `lolwtf.dev` zone is in the filter, which is what lets a route on that zone (oauth2-proxy's authenticated edge today, a Spindrift App's own HTTPRoute when an installation serves one) publish its name. Each cluster's overlay patches in `--fqdn-template={{.Name}}.${SECRET_DOMAIN}`.
...
- ## Cloudflare Tunnel
	...
	- Spindrift has a separate remotely managed wildcard tunnel in `terraform/network/cloudflare/spindrift.tf`; Terraform escrows the generated credential directly into 1Password. No cluster runs its edge Deployment — a Spindrift installation's Target cluster declares one when `reach: public` Apps are served through it.
	- The one name published in `lolwtf.dev` from a cluster is `oauth2.lolwtf.dev`, oauth2-proxy's authenticated edge: its own `oauth2-tls` listener on offsite's `cluster-gateway`, a cert-manager DNS-01 certificate, and an unproxied A record external-dns writes from the route at the gateway's address. When an installation serves Apps in this zone, each App's own HTTPRoute publishes its name the same way — an unproxied A record at the Apps gateway for a private reach, a proxied CNAME at the Spindrift tunnel for a public one; [[Architecture/Spindrift]] covers the reach and auth semantics.
	- A proxied wildcard CNAME answers for every name in `lolwtf.dev`, and a Cloudflare Access application covers it. Both are Terraform's: the tunnel module mints one DNS record per ingress hostname, so the wildcard routing rule the tunnel needs and the wildcard record that claims the whole zone are a single declaration in `terraform/network/cloudflare/spindrift.tf`.
...
- ## DNS
	...
	- Cloudflare zones, records, Access policy, and tunnels are declared under `terraform/network/cloudflare/`. The dedicated `lolwtf.dev` zone carries Spindrift names — the `oauth2` authenticated edge, and every App name an installation mints; ordinary lab and cluster names use `lolwtf.ca`.
