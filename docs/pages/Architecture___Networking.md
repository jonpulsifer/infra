...
- ## Gateway API ingress
	...
	- Each cluster also mints a certificate for its gateway console domain (`${GATEWAY_DOMAIN}` plus `unifi.${GATEWAY_DOMAIN}`), and a `unifi-cert-push` CronJob (`clusters/base/networking/unifi-cert-push/`) delivers it to the site's UniFi gateway over SSH — writing `/data/unifi-core/config/unifi-core.crt`/`.key` and restarting `unifi-core` only when the certificate changed — so the console UI serves the cert-manager certificate instead of its self-signed default. Auth is the console root password from the per-cluster `gateway-ssh` SOPS secret.
- ## external-dns
	- `clusters/base/networking/external-dns/` runs external-dns against provider `cloudflare`, sourcing records from `crd`, `ingress`, and `gateway-httproute`, in `sync` policy with `txtOwnerId: ${CLUSTER_NAME}` (so folly and offsite don't fight over the same zone's TXT ownership records) and `domainFilters` scoped to the cluster's secret domain, `${GATEWAY_ZONE}`, `${SPINDRIFT_DOMAIN}`, `embarrassing.ca`, and `wishin.app` — the last four are the zones Spindrift mints App names in, which is what lets a route on any of them (oauth2-proxy's authenticated edge, a Spindrift App's own HTTPRoute) publish its name. Each cluster's overlay patches in `--fqdn-template={{.Name}}.${SECRET_DOMAIN}`.
