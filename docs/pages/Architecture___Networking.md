...
- ## Cloudflare Tunnel
	...
	- Generated App names under `lolwtf.dev` resolve only to the Spindrift tunnel. The tunnel forwards to the offsite cluster's internal shared Gateway, and the App chart marks its HTTPRoutes as excluded from external-dns so the load-balancer address cannot become an alternate published origin. Cloudflare Access supplies the default Private audience; [[Architecture/Spindrift]] covers exposure semantics and the fail-closed Public boundary.
