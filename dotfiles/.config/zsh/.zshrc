...
# History widget (Up / Ctrl-R): prompt-at-bottom, shrink to content (max ~40%)
export FZF_CTRL_R_OPTS="--layout=default --height=~40%"
export FZF_DEFAULT_COMMAND="fd --type f 2>/dev/null || find ."
# Ctrl-T (paste file path): bat preview; ctrl-/ toggles the preview pane
export FZF_CTRL_T_OPTS="--preview 'bat --style=numbers --color=always --line-range=:200 {} 2>/dev/null' --bind 'ctrl-/:toggle-preview'"
# Alt-C (cd into dir): eza tree preview; fd lists dirs (respects .gitignore)
export FZF_ALT_C_COMMAND="fd --type d --exclude .git 2>/dev/null || find . -type d"
export FZF_ALT_C_OPTS="--preview 'eza --tree --level=2 --color=always {} 2>/dev/null' --bind 'ctrl-/:toggle-preview'"
...
# --- Aliases (from former basic.nix) ---
alias ls='eza'
alias ll='eza -l'
alias la='eza -la'
alias htop='btop; echo "stop using [h]top, prefer btop"'
alias tree='eza --tree'
alias diff=delta
alias chctx='kubectx'
alias chns='kubens'
