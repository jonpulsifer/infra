...
resource "google_bigquery_table" "outliers" {
  dataset_id          = google_bigquery_dataset.audit_anomalies.dataset_id
  table_id            = "outliers"
  deletion_protection = false
  ...
  schema = jsonencode([
    ...
    { name = "outlier_distance", type = "FLOAT", mode = "REQUIRED" },
    { name = "detected_at", type = "TIMESTAMP", mode = "REQUIRED" },
    { name = "principal_email", type = "STRING", mode = "REQUIRED" },
    { name = "method_name", type = "STRING", mode = "REQUIRED" },
    { name = "resource_name", type = "STRING", mode = "NULLABLE" },
...
resource "google_bigquery_data_transfer_config" "outlier_detection" {
  display_name   = "Daily outlier detection via ML.PREDICT"
  location       = "US"
  data_source_id = "scheduled_query"
  ...
  params = {
    query = file("${path.module}/sql/predict.sql")
  }
}
