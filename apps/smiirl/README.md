...
| `TZ`              | `Canada/Atlantic` | clock the daily step fires on    |
...
- `GET /api/state` —
  `{"cells","number","updatedAt","daily":{"step","at","next"},"device":{"lastPoll","lastStatus","online"}}`;
  `number` is `null` when the cells are not a plain number, `next` is `null`
