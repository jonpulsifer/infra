...
describe('the line a thread watches while it waits', () => {
  ...
  test('is taken out of the thread when the turn starts', async () => {
    ...
    // Nothing of the wait is left above the answer, and the redraw that would
    // have put it back is cancelled with it.
    expect(surface.linesIn(start.id)).toEqual([]);
    expect(clock.pendingTimers).toBeLessThanOrEqual(1);
  });
