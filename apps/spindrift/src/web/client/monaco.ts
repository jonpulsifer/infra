...
/** The types the editor checks a handler's `fetch(request, env, ctx)` against. */
const ENV_DTS = `
...
/** Monaco's loader sets `window.monaco` and passes its callback nothing. */
...
// `getComputedStyle` returns a custom property's `light-dark()` unresolved, so
// the regex picks a half. Any other colour goes through a canvas to become hex.
const LIGHT_DARK =
...
/** Rerun on every theme change, since each token resolves to a different hex. */
