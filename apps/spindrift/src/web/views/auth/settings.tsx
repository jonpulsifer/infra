 ...
 *
 * Passkeys are the account root, so every button on this screen begins with a
 * fresh passkey assertion in `auth-client.ts`. The view does not infer that a
 * session is fresh and never offers removal of the final passkey.
 */
...
export function CredentialSettingsView({
  ...
  return (
    <main className="mx-auto flex w-full max-w-[760px] flex-col gap-5 px-5 py-8">
      <div>
        <h1 className="text-xl font-semibold tracking-tight">Authentication</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Every change requires a fresh assertion from an enrolled passkey.
        </p>
      </div>
