...
#      (its trunk refusing, its Secret absent, or two calls already on it) is
#      held in a spam sink with the ten-minute cap and its hangup handler, the
#      agent's leg carries the mode and caller headers its pre-dial handler
#      adds, a third stranger on a line screening two is refused unanswered,
#      a fourth held caller hears goodbye, the INVITE to the handset carries
#      the Alert-Info its pre-dial handler adds, every trunk takes voip.ms's
#      DID-form INVITE by its own X-Dest-User header and never by its From
#      user, and every prompt the dialplan plays is in the ConfigMap mounted
#      for it.
#
...
# breaks a real call while every check here keeps passing. FORBIDDEN_MODULES
# are the exception: the boot check requires those absent.
check_no_pjsip_noload() {
  local offenders
  offenders=$(awk -v forbidden="${FORBIDDEN_MODULES[*]}" '
    BEGIN { m = split(forbidden, f, " "); for (i = 1; i <= m; i++) refused[f[i]] = 1 }
    /^[[:space:]]*noload[[:space:]]*=/ {
      mod = $0
      sub(/^[[:space:]]*noload[[:space:]]*=[[:space:]]*/, "", mod)
      sub(/[[:space:]]*;.*/, "", mod)
      sub(/[[:space:]]+$/, "", mod)
      if (mod ~ /^(res_pjsip|chan_pjsip)/ && !(mod in refused)) print FILENAME ":" FNR ": " $0
    }
...
# ElevenLabs would; `trolling` stands in for a call the agent is on, and
# `earl` enters as the handset's *29 does.
...
# refusing here, lands in a sink with the troll's held-time handler popped; a
# caller over the agent's two calls is never dialled. Without the Secret,
# pbx-env's `off` sends every caller straight to a sink. The handset's desk
# demo never lands in a sink.
check_troll() {
  local log="$SITE_DIR/asterisk.log" miss offset want
  ...
  expect_route troll '"PJSIP/line4,' \
    ...
    '"SPYGROUP=troll"' '"CHANNEL(hangup_handler_push)=spam-held,s,1"' \
...
check_trunk_identifies() {
  ...
  for id in $(ast 'pjsip show identifies' | awk '$1 == "Identify:" && $2 !~ /^</ { print $2 }'); do
    ep=${id#*/}
    id=${id%%/*}
    show=$(ast "pjsip show identify $id")
    if awk '$1 == "Match:" && $2 !~ /^</ { found = 1 } END { exit !found }' <<<"$show"; then
      wrong+=("identify $id for $ep matches by address")
    fi
    match=$(awk '$1 == "Header:" { $1 = ""; sub(/^ +/, ""); print; exit }' <<<"$show")
    [[ -n $match ]] || continue
    name=${match%%:*}
    name=${name%"${name##*[![:space:]]}"}
    value=${match#*:}
    value=${value#"${value%%[![:space:]]*}"}
    # PJSIP finds a header by name without regard to case.
    if [[ ${name,,} != "${IDENTIFY_HEADER,,}" ]]; then
      wrong+=("identify $id for $ep matches header '$name', not $IDENTIFY_HEADER")
    fi
    if [[ -z $value ]]; then
      wrong+=("identify $id for $ep matches an empty value")
      continue
    fi
    if [[ -n ${owner[$value]:-} ]]; then
      wrong+=("identifies for ${owner[$value]} and $ep both match '$value', so Asterisk picks one")
    fi
    owner[$value]=$ep
    n=$((n + 1))
    endpoints[n]=$ep
